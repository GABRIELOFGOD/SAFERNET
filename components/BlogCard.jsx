...
const BlogCard = ({ image, topic, content, author, date }) => {
  ...
  return (
    <div className='md:w-[300px] w-full duration-200 bg-white shadow-md rounded-md overflow-hidden mb-3'>
      {image && (
        <div className='max-h-[200px] overflow-hidden'>
          <img className='w-full h-auto object-cover hover:scale-105 duration-300' src={image} alt="blog image" />
        </div>
      )}
      <div className='p-5 shadow-sm'>
        <p className="font-bold capitalize text-primary">{titleTrunc(topic)}</p>
        ...
        <p className='text-[12px]' dangerouslySetInnerHTML={{ __html: sanitizedContent }}></p>
      </div>
    </div>
  );
};
