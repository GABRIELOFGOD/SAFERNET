...
const Header = () => {
  ...
  return (
    <div className='flex sticky z-50 top-0 justify-between px-6 md:px-20 py-5 bg-white uppercase shadow-sm'>
      <Link onClick={closeNav} to='/' className={`md:text-4xl text-3xl my-auto font-extrabold`}><img className='md:h-[50px] h-[40px]' src='/svgs/logo.svg' /></Link>
      <ul className={`md:flex-row flex flex-col activa duration-300 text-white md:text-black md:top-0 md:w-fit w-full left-0 md:py-0 py-5 absolute md:relative gap-5 md:gap-3 my-auto ${navShow ? 'top-[80px] bg-primary' : 'top-[-500px]'}`}>
        ...
        <li onClick={closeNav} className='w-full my-auto md:w-fit'><NavLink to='media' className='hover:bg-button text-[12px] p-3 hover:text-white font-bold rounded-sm duration-300'>media</NavLink></li>
        {/* <li><ListItems
          title='updates'
          lists={[
            {
        ...
        <li onClick={closeNav} className='my-auto mb-4 md:mb-auto'><NavLink className='hover:bg-button hover:text-white text-[12px] p-3 font-bold rounded-sm duration-300' target='_blank' to='https://legal.thesafernet.org'>Safelegal</NavLink></li>
